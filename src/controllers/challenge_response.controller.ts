            ...
            if (id) {
                ...
                data = await this.crudService.findOne(ideas, {
                    attributes: [
                        ...
                        // [
                        //     db.literal(`(SELECT mentorTeamOrg.organization_name FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id LEFT OUTER JOIN organizations AS mentorTeamOrg ON mentorTeam.organization_code = mentorTeamOrg.organization_code WHERE ideass.team_id =  \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'organization_name'
                        // ],
                        // [
                        //     db.literal(`(SELECT mentorTeamOrg.organization_code FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id LEFT OUTER JOIN organizations AS mentorTeamOrg ON mentorTeam.organization_code = mentorTeamOrg.organization_code WHERE ideass.team_id = \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'organization_code'
                        // ],
                        // [
                        //     db.literal(`(SELECT full_name FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id WHERE ideass.team_id = \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'mentor_name'
                        // ],
                        "created_by",
                        "updated_by",
                        "created_at",
                ...
                try {
                    const responseOfFindAndCountAll = await this.crudService.findAndCountAll(ideas, {
                        where: {
                            [Op.and]: [
                                condition
                            ]
                        ...
                        attributes: [
                            ...
                            // ],
                            // [
                            //     db.literal(`(SELECT mentorTeamOrg.district FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id LEFT OUTER JOIN organizations AS mentorTeamOrg ON mentorTeam.organization_code = mentorTeamOrg.organization_code WHERE ideass.team_id = \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'district'
                            // ],
                            // [
                            //     db.literal(`(SELECT mentorTeamOrg.category FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id LEFT OUTER JOIN organizations AS mentorTeamOrg ON mentorTeam.organization_code = mentorTeamOrg.organization_code WHERE ideass.team_id = \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'category'
                            // ],
                            // [
                            //     db.literal(`(SELECT full_name FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id WHERE ideass.team_id = \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'mentor_name'
                            // ],
                            // [
                            //     db.literal(`(SELECT mobile FROM ideass AS ideass LEFT OUTER JOIN teams AS team ON ideas.team_id = team.team_id LEFT OUTER JOIN mentors AS mentorTeam ON team.mentor_id = mentorTeam.mentor_id WHERE ideass.team_id = \`ideas\`.\`team_id\` GROUP BY ideas.team_id)`), 'mobile'
                            // ],
                            "initiated_by",
                            "created_at",
                            "updated_at",
